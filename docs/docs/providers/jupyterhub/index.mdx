...
For general information about running Jupyter MCP Server as a Jupyter Server extension, see the [Jupyter Server Extension documentation](/transports/streamable-http-extension).
...
```bash
pip install "jupyter-mcp-server>=0.15.0" "jupyterlab==4.4.1" "jupyter-collaboration==4.0.2" "ipykernel" "pycrdt"
```
...
MCP clients must authenticate using the JupyterHub API token.

#### Claude Code

```bash
claude mcp add jupyter --transport http \
  https://your-jupyterhub.domain/user/<username>/mcp \
...
```json
{
  "mcpServers": {
    "jupyter": {
      ...
      "env": {
        "JUPYTERHUB_AUTH_HEADER": "Bearer your-api-token-here"
      }
    }
  }
}
...
For general extension troubleshooting, see the [Jupyter Server Extension documentation](/transports/streamable-http-extension).
