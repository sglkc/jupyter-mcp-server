...
| `SANDBOX_VARIANT` | Code sandbox variant (see [Sandbox Variants](#sandbox-variants)) | `jupyter` / `kaggle` / `monty` / `colab` / `modal` / `docker` / `eval` / `datalayer` | `jupyter` |
| `RUNTIME_PROXY_TOKEN` | Proxy token used by the `colab` sandbox variant (`colab-runtime-proxy-token`) | `ya29...` | `None` |
| `RUNTIME_CHANNELS_URL` | For the `kaggle` and `colab` engines, notebook session websocket channels URL used to derive `RUNTIME_URL` and `RUNTIME_ID` | `wss://.../api/kernels/.../channels?...` | `None` |
...
By default the server executes code through `jupyter-kernel-client` against a
Jupyter Server (`SANDBOX_VARIANT=jupyter`). Setting `SANDBOX_VARIANT` to any
other value routes execution through the
[code-sandboxes](https://github.com/datalayer/code-sandboxes) package via a
`SandboxKernel` adapter, so the same notebook tools can run code on additional
backends.
...
|--------|-------------|---------------|---------------|
| `jupyter` (default) | Jupyter Server via `jupyter-kernel-client` | — | `JUPYTER_URL`, `JUPYTER_TOKEN` (or `RUNTIME_*` + `DOCUMENT_*`) |
| `kaggle` | Kaggle notebook runtime | `jupyter-mcp-server[kaggle]` | Default batch mode: `KAGGLE_API_TOKEN` (or Kaggle credentials). Interactive mode: `RUNTIME_URL` (+ `KAGGLE_API_TOKEN`, or `RUNTIME_ID`). Optional accelerator: `SANDBOX_GPU`. |
| `monty` | In-process secure Python interpreter ([Monty](https://github.com/pydantic/monty)) | `jupyter-mcp-server[monty]` | — |
| `colab` | Google Colab runtime | `jupyter-mcp-server[colab]` | `RUNTIME_URL`, `RUNTIME_ID`, `RUNTIME_PROXY_TOKEN` |
...
| `datalayer` | Datalayer cloud runtime | `jupyter-mcp-server[sandboxes]` | `RUNTIME_URL`, `RUNTIME_TOKEN`, `SANDBOX_ENVIRONMENT` |
...
# Default batch mode (no runtime URL required):
KAGGLE_API_TOKEN=...
SANDBOX_GPU=T4
# Interactive mode (optional):
# RUNTIME_URL=https://kkb-production.jupyter-proxy.kaggle.net/k/12345678/eyJ.../proxy
# ...or connect to an existing kernel by also setting RUNTIME_ID:
# RUNTIME_ID=11e073f0-e82d-4029-be8d-3918f7ed1a9e
```
...
Options:
  ...
  --runtime-proxy-token TEXT          Proxy token for the colab sandbox variant (default: None)
  --runtime-channels-url TEXT         WebSocket channels URL for the kaggle or colab sandbox variants (default: None)
