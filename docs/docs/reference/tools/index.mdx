...
- Input:
  ...
  - `delivery`(string, optional): `"image"` (default) returns MCP `ImageContent`; `"path"` writes under `JUPYTER_MCP_ARTIFACT_DIR` and returns the absolute path only (no base64). Path mode fails clearly if the env var is unset.
