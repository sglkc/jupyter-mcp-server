...
- Input:
  ...
  - `max_edge`(int, optional): Max longest side in pixels after resize; `0` uses `JUPYTER_MCP_IMAGE_MAX_EDGE` (default 1024)
  - `max_bytes`(int, optional): Soft max raw bytes after compression; `0` uses `JUPYTER_MCP_IMAGE_MAX_BYTES` (default 150000)
...
- Returns: Metadata text plus one `ImageContent` (`delivery=image`), or path text (`delivery=path`), or an error message
