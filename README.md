...
| `execute_multiple_cells`   | Execute an inclusive cell range; skips non-code; stops on first error. Supports the same `stream` / `progress_interval` options as `execute_cell`. |
...
> 1. **Image Output**: Plot/image cell outputs are **placeholders by default** on `execute_cell` / `read_cell`. Use `read_cell_image` when the model should see a figure:
