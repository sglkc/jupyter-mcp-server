...
class LocalBackend(Backend):
    ...
    async def get_notebook_content(self, path: str) -> dict[str, Any]:
        ...
        """
        model = await ensure_async(self.contents_manager.get(path, type="notebook", content=True))
    ...
    async def _list_notebooks_recursive(self, path: str, notebooks: list[str]) -> None:
        ...
        try:
            model = await ensure_async(self.contents_manager.get(path, content=True))
    ...
    async def notebook_exists(self, path: str) -> bool:
        ...
        """
        try:
            await ensure_async(self.contents_manager.get(path, content=False))
            return True
        except Exception:
            return False
    ...
    async def create_notebook(self, path: str) -> dict[str, Any]:
        ...
        """
        model = await ensure_async(self.contents_manager.new(path=path))
        return model["content"]
    ...
    async def append_cell(
        ...
        await ensure_async(
            self.contents_manager.save({"type": "notebook", "content": content}, path)
        )
        ...
        while True:
            ...
            try:
                msg = await asyncio.wait_for(
                    asyncio.to_thread(client.get_iopub_msg, timeout=1), timeout=2
                )
    ...
    async def list_kernels(self) -> list[dict[str, Any]]:
        ...
        return [
            {"id": kid, "name": self.kernel_manager.get_kernel(kid).kernel_name}
