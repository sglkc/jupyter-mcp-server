            ...
            if "-" in part:
                ...
                if len(range_parts) == 2:
                    ...
                    if not end_str:
                        ...
                        if start >= max_cells:
                            raise ValueError(
                                f"Cell index {start} is out of range. Notebook has {max_cells} cells."
                            )
                    else:
                        try:
                            end = int(end_str)
                ...
                for i in range(start, end + 1):
                    if i >= max_cells:
                        raise ValueError(
                            f"Cell index {i} is out of range. Notebook has {max_cells} cells."
                        )
                    result.add(i)
            else:
                # Handle single index
                ...
                if index >= max_cells:
                    raise ValueError(
                        f"Cell index {index} is out of range. Notebook has {max_cells} cells."
                    )
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if notebook_name not in notebook_manager:
            raise ValueError(
                f"Notebook '{notebook_name}' is not connected. All currently connected notebooks: {list(notebook_manager.list_all_notebooks().keys())}"
            )
        ...
        prompt_list = [
            f"USER Cite cells {parsed_indices} from notebook {notebook_name}, here are the cells:"
        ]
