    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                pytest.skip(
                    f"Test {func.__name__} timed out ({timeout_seconds}s) - known platform limitation"
                )
            except Exception as e:
                # Check if it's a network timeout related to Windows
                if "ReadTimeout" in str(e) or "TimeoutError" in str(e):
                    pytest.skip(
                        f"Test {func.__name__} hit network timeout - known platform limitation: {e}"
                    )
                raise
...
class MCPClient:
    ...
    def _extract_text_content(result):
        ...
        try:
            logging.debug(
                f"_extract_text_content: result type={type(result)}, has content={hasattr(result, 'content')}, is tuple={isinstance(result, tuple)}, is list={isinstance(result, list)}"
            )

            # Handle tuple results (content, metadata)
            if isinstance(result, tuple) and len(result) >= 2:
                logging.debug(
                    f"_extract_text_content: handling tuple, first element type={type(result[0])}"
                )
            ...
            if hasattr(result, "content") and result.content and len(result.content) > 0:
                ...
                if all(isinstance(item, types.TextContent) for item in result.content):
                    ...
                    if len(result.content) > 1:
                        ...
                        logging.debug(
                            f"_extract_text_content: extracted {len(texts)} TextContent items as JSON list"
                        )
                        return text
                    else:
                        text = result.content[0].text
                        logging.debug(
                            f"_extract_text_content: extracted from result.content[0].text, length={len(text)}"
                        )
                        return text
            # Handle list results directly
                ...
                if all(isinstance(item, types.TextContent) for item in result):
                    ...
                    if len(result) > 1:
                        ...
                        text = result[0].text
                        logging.debug(
                            f"_extract_text_content: extracted from list[0].text, length={len(text)}"
                        )
                        return text
        except (AttributeError, IndexError, TypeError) as e:
            logging.debug(f"_extract_text_content error: {e}, result type: {type(result)}")

        logging.debug("_extract_text_content: returning None, could not extract")
        return None
    ...
    def _get_structured_content_safe(self, result):
        ...
        if content is None:
            ...
            logging.debug(
                f"_get_structured_content_safe: text_content={text_content[:200] if text_content else None!r}"
            )
            if text_content:
                ...
                try:
                    ...
                    logging.debug(
                        f"_get_structured_content_safe: JSON parsed successfully, type={type(parsed)}"
                    )
                    ...
                    try:
                        ...
                        logging.debug(
                            f"_get_structured_content_safe: ast.literal_eval succeeded, type={type(parsed)}, value={parsed!r}"
                        )
                        return {"result": parsed}
                    except (ValueError, SyntaxError):
                        # Plain text - return as-is
                        logging.debug(
                            "_get_structured_content_safe: Plain text, wrapping in result dict"
                        )
                ...
                if hasattr(result, "content") and result.content:
                    ...
                    if content_list:
                        logging.debug(
                            f"_get_structured_content_safe: extracted {len(content_list)} items from mixed content"
                        )
    ...
    async def _call_tool_safe(self, tool_name, arguments=None):
        ...
        try:
            ...
            text_content = self._extract_text_content(result)
            if text_content and (
                "Error executing tool" in text_content
                or "is out of range" in text_content
                or "not found" in text_content
            ):
            ...
            if structured_content:
                ...
                if result_value:
                    ...
                    if error_text and (
                        "[ERROR:" in error_text
                        or "is out of range" in error_text
                        or "not found" in error_text
                    ):
                        logging.warning(
                            f"Tool {tool_name} returned error in structured result: {error_text[:100]}"
                        )
                        return None
    ...
    async def restart_notebook(self, notebook_name):
        result = await self._session.call_tool(
            "restart_notebook", arguments={"notebook_name": notebook_name}
        )  # type: ignore
        return self._extract_text_content(result)
    ...
    async def unuse_notebook(self, notebook_name):
        result = await self._session.call_tool(
            "unuse_notebook", arguments={"notebook_name": notebook_name}
        )  # type: ignore
        return self._extract_text_content(result)
    ...
    async def insert_execute_code_cell(self, cell_index, cell_source, timeout=90):
        result = await self._call_tool_safe(
            "insert_execute_code_cell",
            {"cell_index": cell_index, "cell_source": cell_source, "timeout": timeout},
        )
    ...
    async def read_cell(self, cell_index, include_outputs=True):
        result = await self._call_tool_safe(
            "read_cell", {"cell_index": cell_index, "include_outputs": include_outputs}
        )
        return self._get_structured_content_safe(result) if result else None
    ...
    async def move_cell(self, source_index: int, target_index: int):
        result = await self._call_tool_safe(
            "move_cell", {"source_index": source_index, "target_index": target_index}
        )
        return self._get_structured_content_safe(result) if result else None
    ...
    async def delete_cell(self, cell_indices: list[int], include_source: bool = True):
        result = await self._call_tool_safe(
            "delete_cell", {"cell_indices": cell_indices, "include_source": include_source}
        )
        return self._get_structured_content_safe(result) if result else None
    ...
    async def execute_cell_with_progress(self, cell_index):
        result = await self._call_tool_safe(
            "execute_cell_with_progress", {"cell_index": cell_index}
        )
        structured = self._get_structured_content_safe(result) if result else None
    ...
    async def execute_cell(
        self, cell_index, timeout_seconds=300, stream=False, progress_interval=5
    ):
        result = await self._call_tool_safe(
            "execute_cell",
            {
    ...
    async def overwrite_cell_source(self, cell_index, cell_source):
        result = await self._call_tool_safe(
            "overwrite_cell_source", {"cell_index": cell_index, "cell_source": cell_source}
        )
        return self._get_structured_content_safe(result) if result else None
    ...
    async def execute_code(self, code, timeout=60):
        result = await self._session.call_tool(
            "execute_code", arguments={"code": code, "timeout": timeout}
        )  # type: ignore
    ...
    async def jupyter_cite(self, prompt, cell_indices, notebook_name=""):
        prompt = await self._session.get_prompt(
            "jupyter_cite",
            arguments={
                "prompt": prompt,
